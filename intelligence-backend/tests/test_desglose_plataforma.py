...
def test_aggregate_suma_por_plataforma_y_descarta_lo_desconocido():
    rows = [
        {"publisher_platform": "facebook", "country": "GT", "spend": "6", "impressions": "100", "reach": "80", "clicks": "3"},
        {"publisher_platform": "facebook", "country": "MX", "spend": "4", "impressions": "50", "reach": "40", "clicks": "1"},
        {"publisher_platform": "instagram", "country": "GT", "spend": "2", "impressions": "30", "reach": "25", "clicks": "1"},
        {"publisher_platform": "unknown_future_platform", "country": "GT", "spend": "99", "impressions": "1", "reach": "1", "clicks": "1"},
    ]
    result = report_builder._aggregate_platform_breakdown(rows, country_code=None)
    assert result == [
...
def test_aggregate_filtra_por_pais_cuando_el_reporte_lo_pide():
    rows = [
        {"publisher_platform": "facebook", "country": "GT", "spend": "6", "impressions": "100", "reach": "80", "clicks": "3"},
        {"publisher_platform": "facebook", "country": "MX", "spend": "4", "impressions": "50", "reach": "40", "clicks": "1"},
    ]
    result = report_builder._aggregate_platform_breakdown(rows, country_code="GT")
    assert len(result) == 1
    assert result[0]["spend"] == 6.0
...
def test_aggregate_omite_plataformas_sin_gasto():
    ...
    rows = [{"publisher_platform": "messenger", "country": "GT", "spend": "0", "impressions": "5", "reach": "5", "clicks": "0"}]
    assert report_builder._aggregate_platform_breakdown(rows, None) == []


def test_aggregate_lista_vacia_si_meta_no_devuelve_nada():
    assert report_builder._aggregate_platform_breakdown([], None) == []
...
def test_build_report_data_incluye_el_desglose_por_plataforma(monkeypatch, tenant_a, factory):
    ...
    async def fake_get_platform_breakdown_with_fallback(tokens, ad_account_id, date_from, date_to):
        return [
            {"publisher_platform": "facebook", "country": "GT", "spend": "7", "impressions": "70", "reach": "60", "clicks": "2"},
            {"publisher_platform": "instagram", "country": "GT", "spend": "3", "impressions": "30", "reach": "25", "clicks": "1"},
        ]
...
def test_build_report_data_convierte_el_spend_del_desglose(monkeypatch, tenant_a, factory):
    ...
    async def fake_get_platform_breakdown_with_fallback(tokens, ad_account_id, date_from, date_to):
        return [{"publisher_platform": "facebook", "country": "GT", "spend": "10", "impressions": "1", "reach": "1", "clicks": "1"}]
...
def test_get_platform_breakdown_manda_los_dos_breakdowns(monkeypatch):
    captured = {}

    async def fake_get_all(client, path, token, params):
        captured.update(params)
        captured["path"] = path
        return [{"publisher_platform": "facebook", "country": "GT", "spend": "1"}]
    ...
    assert captured["breakdowns"] == "publisher_platform,country"
    assert captured["level"] == "account"
    assert captured["path"] == "act_1/insights"
    assert result == [{"publisher_platform": "facebook", "country": "GT", "spend": "1"}]
...
def test_get_platform_breakdown_with_fallback_prueba_el_siguiente_token(monkeypatch):
    async def fake_get_platform_breakdown(token, ad_account_id, date_from, date_to):
        ...
        return [{"publisher_platform": "facebook", "country": "GT", "spend": "5"}]
    ...
    assert result == [{"publisher_platform": "facebook", "country": "GT", "spend": "5"}]
